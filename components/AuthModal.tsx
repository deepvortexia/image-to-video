    ...
    <div className="auth-modal-overlay" onClick={onClose}>
      <div className="auth-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="auth-modal-header">
          <img src="/deepgoldremoveetiny.png" alt="Deep Vortex Logo" className="auth-modal-logo" />
          ...
          <div className="auth-modal-content">
            ...
            <form onSubmit={handleEmailSignIn}>
              <input type="email" placeholder="Enter your email" value={email}
                onChange={(e) => setEmail(e.target.value)} className="auth-input" disabled={isLoading} />
              <button type="submit" className="auth-btn auth-btn-email" disabled={isLoading}>
                {isLoading ? 'Sending...' : '✉️ Send Magic Link'}
              </button>
            </form>
            {message && (
              <div className={`auth-message ${message.includes('Check') ? 'success' : 'error'}`}>{message}</div>
            )}
          </div>
        ) : (
          <div className="auth-modal-content">
