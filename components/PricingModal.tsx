...
export const PricingModal = ({ isOpen, onClose }: PricingModalProps) => {
  ...
  const handlePurchase = async (pack: PricingPack) => {
    ...
    try {
      ...
      if (!session) {
        setError(refreshError ? 'Session refresh failed. Please sign in again.' : 'Please sign in to purchase credits')
        setIsLoading(false)
        return
      }
          ...
          {pricingPacks.map((pack) => (
            <div key={pack.name} className={`pricing-card ${pack.popular ? 'popular' : ''}`}>
              ...
              <div className="pricing-card-body">
                ...
                <div className="pricing-perunit">${(pack.price / pack.credits).toFixed(3)} per credit</div>
              </div>
              <button className="pricing-btn" onClick={() => handlePurchase(pack)} disabled={isLoading}>
