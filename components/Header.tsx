...
import { VideoFavoritesModal } from './VideoFavoritesModal'
import './Header.css'
  ...
  return (
    <>
      <header className="hub-header" role="banner">
        ...
        <h1 className="brand-title-text">AI Image to Video</h1>
        <p className="primary-tagline">🎬 Transform your images into stunning AI videos</p>
        ...
        <div className="hub-pills-container">
          {user ? (
            ...
            <button className="hub-pill credits-pill" style={{ cursor: 'pointer' }}
              onClick={() => setShowAuthModal(true)} title="Sign in to get free credits">
              <span className="pill-icon">🏆</span>
          ...
          {user && (
            <button className="hub-pill buy-credits-pill" onClick={() => setShowFavoritesModal(true)} title="View saved videos">
              <span className="pill-icon">⭐</span>
          ...
          {user ? (
            ...
            <button className="hub-pill signin-pill" onClick={() => setShowAuthModal(true)}
              disabled={loading && !loadingTimeout} title="Sign in to get credits">
              <span className="pill-icon">🔐</span>
      ...
      <VideoFavoritesModal isOpen={showFavoritesModal} onClose={() => setShowFavoritesModal(false)} />
    </>
  )
}
